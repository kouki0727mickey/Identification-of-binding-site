...
def seq2hist(r_name, w_name, normalize=True, window_size=5, base=dict(
    {'A': 0, 'T': 1, 'C': 2, 'G': 3, 'N': 0, 'R': 0, 'M': 0, 'Y': 1, 'W': 0, 'K': 1, 'S': 1, 'B': 1, 'D': 0, 'H': 0,
     'V': 1, 'L': 0, 'I': 0, 'F': 0, 'Q': 0, 'P': 0, 'E': 0, 'X': 0, 'J': 0, 'X': 0, 'Z': 0})):
    """
    シーケンスをヒストグラムに変換する関数
    引数
...
def seq2histFromcsv(nameSource, w_name, normalize=True, window_size=5, base=dict(
    {'A': 0, 'T': 1, 'C': 2, 'G': 3, 'N': 0, 'R': 0, 'M': 0, 'Y': 1, 'W': 0, 'K': 1, 'S': 1, 'B': 1, 'D': 0, 'H': 0,
     'V': 1, 'L': 0, 'I': 0, 'F': 0, 'Q': 0, 'P': 0, 'E': 0, 'X': 0, 'J': 0, 'X': 0, 'Z': 0})):
    """
    シーケンスをヒストグラムに変換する関数
    ...
    def add_greater(data):
        data_gre = []
        count = 0
        for dt in data:
            count += 1
            if count == 1:
                if dt[0] == 'Accession' or 'accession':
                    dt[0] = dt[0]
                else:
                    dt[0] = '>' + dt[0]
            else:
    ...
    def del_dot(data):
        ...
        for dt in data:
            ...
            for d in dt[1:]:
                dgre.append(d)
            data_dot.append(dgre)
        return data_dot



    data = pd.read_csv(nameSource, header=None, engine='python')
    data = data.values

    if data[1][0][0] != '>':
        data = add_greater(data)
        print(r_file1 + '：accession number に > がないため追加')

    if data[1][0].find('.') != -1:
        data = del_dot(data)
        print(r_file1 + '：accession number に . があるため削除')

    #print(data[0])
    #print(data[0][0])
    header = []
    if data[0][0][0] == '>':  # 二次元配列における(0,0)の1文字目が'>' すなわちaccession numberなら headerなし
        print("header データなし")
        print("ただいまheader データなしのcsvファイルには対応していません。")
        print("csvファイルにheaderを追加するか、fastaファイルを入力としてください。")
        input()
        exit()
    else:
        print("header データあり")
        for i in data[0]:  # header あり
            header.append(i)
        data = data[1:, :]
    #print(header)

    for i, head in enumerate(header):
        if head == 'Accession' or head == 'accession' or head == 'acc':
            accession_column = i
        if head == 'Sequence' or head == 'sequence' or head == 'Seq' or head == 'seq':
            sequence_column = i
    ...
    #with open(r_name, 'r') as r_file:
    cnt = 0
    print(data[:, sequence_column])
    for line in data[:, sequence_column]:
        #line = r_file.readline()
        if not line:
            break
        hist = [0] * ncol
...
def pseq2hist(r_name, w_name, normalize=True, window_size=5, base=dict({'A':0, 'G':1, 'M':2, 'S':3, 'C':4, 'H':5, 'N':6, 'T':7, 'D':8, 'I':9, 'P':10,'V':11, 'E':12,'K':13,'Q':14,'W':15,'F':16,'L':17,'R':18,'Y':19})):
    ...
    with open(r_name, 'r') as r_file:
        ...
        while 1:
            ...
            com = range(len(line))
            com_ist = list(itertools.combinations(com, window_size))
            com_ist = filter(lambda x: x[0] <=58 , com_ist)

            for k in com_ist:
                at = sum([np.power(type, window_size - 1 - k.index(j)) * int(numeric_seq[j]) for j in k])
                hist[at] += 1
            writer.writerow(hist)
